...
gulp.task('copy:libs', function () {
  console.log('---------- Копирование шрифтов');
  return gulp.src(dirs.srcPath + '/libs/**/*')
    ...
    .pipe(gulp.dest(dirs.buildPath + '/libs'));
});
...
gulp.task('build', function (callback) {
  gulpSequence(
    ['clean'],
    ['style', 'style:single', 'js', 'copy:css', 'copy:img', 'copy:js', 'copy:fonts', 'copy:libs'],
    'html',
    callback
  );
});
...
gulp.task('serve', ['build'], function() {
  ...
  gulp.watch('/libs/*.{ttf,woff,woff2,eot,svg}', {cwd: dirs.srcPath}, ['watch:libs']);
  // Слежение за html
  gulp.watch([
    '*.html',
    '_include/*.html',
